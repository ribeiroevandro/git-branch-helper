...
# Git Branch Helper para Bash
# Função para criar branches Git com padrão personalizado

# Carregar função de configuração se existir
if [ -f "$(dirname "${BASH_SOURCE[0]}")/git_branch_config.bash" ]; then
    source "$(dirname "${BASH_SOURCE[0]}")/git_branch_config.bash"
fi
...
create_branch() {
    ...
    local branch_type="${positional_args[0]:-}"
    local branch_name="${positional_args[@]:1}"
    ...
    if [[ -z "$branch_type" ]]; then
        ...
        echo "  7) test    - Testes"
        read -rp "📝 Digite o número ou nome do tipo de branch: " branch_type
    fi
    ...
    if [[ -z "$branch_name" ]]; then
        read -rp "📝 Digite o nome da branch (ex: migração de tela xpto): " branch_name
    fi
    ...
    if [[ $auto_confirm -eq 0 ]]; then
        read -rp "✅ Criar esta branch? [Y/n]: " confirm
...
# Autocompletar para Bash (requer bash-completion)
_create_branch_completions() {
    local cur="${COMP_WORDS[COMP_CWORD]}"
    local prev="${COMP_WORDS[COMP_CWORD-1]}"
    
    if [[ $COMP_CWORD -eq 1 ]]; then
        COMPREPLY=($(compgen -W "feat fix chore docs style refactor test 1 2 3 4 5 6 7 -y --yes" -- "$cur"))
    fi
}

complete -F _create_branch_completions create_branch